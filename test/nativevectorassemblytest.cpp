...
int main(int argc, char *argv[])
{
	...
	for(PetscInt k = start[2]; k < start[2]+lsize[2]; k++)
		for(PetscInt j = start[1]; j < start[1]+lsize[1]; j++)
			...
			{
				assert(u2vec[k][j][i] == u1.vals[m2.localFlattenedIndexAll(k+1,j+1,i+1)]);
			}

	// iterate over ghost points, see if they're zero
	for(sint j = 0; j < u1.sz[1]+2; j++) {
		for(sint i = 0; i < u1.sz[0]+2; i++)
			assert(u1.vals[m2.localFlattenedIndexAll(0,j,i)] == 0);
	}
	for(sint j = 0; j < u1.sz[1]+2; j++) {
		for(sint i = 0; i < u1.sz[0]+2; i++)
			assert(u1.vals[m2.localFlattenedIndexAll(u1.start+u1.sz[2],j,i)] == 0);
	}

	for(sint k = 0; k < u1.sz[2]+2; k++) {
		for(sint i = 0; i < u1.sz[0]+2; i++)
			assert(u1.vals[m2.localFlattenedIndexAll(k,0,i)] == 0);
	}
	for(sint k = 0; k < u1.sz[2]+2; k++) {
		for(sint i = 0; i < u1.sz[0]+2; i++)
			assert(u1.vals[m2.localFlattenedIndexAll(k,u1.start+u1.sz[1],i)] == 0);
	}

	for(sint k = 0; k < u1.sz[2]+2; k++) {
		for(sint j = 0; j < u1.sz[1]+2; j++)
			assert(u1.vals[m2.localFlattenedIndexAll(k,j,0)] == 0);
	}
	for(sint k = 0; k < u1.sz[2]+2; k++) {
		for(sint j = 0; j < u1.sz[1]+2; j++)
			assert(u1.vals[m2.localFlattenedIndexAll(k,j,u1.start+u1.sz[0])] == 0);
	}

	// Matrix test

	const sreal dx = m2.gcoords(0,2) - m2.gcoords(0,1);
	printf("Diag val = %f, offdiag val = %f\n", 6.0/(dx*dx), -1.0/(dx*dx));
	SMat A1 = pde.computeLHS(&m2);
