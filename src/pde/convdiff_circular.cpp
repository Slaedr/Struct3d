...
ConvDiffCirc::ConvDiffCirc(const sreal advel, const sreal diffc) : mu{diffc}, bmag(advel)
{
 ...
 */
int ConvDiffCirc::computeLHSPetsc(const CartMesh *const m, DM da, Mat A) const
{
...
SMat ConvDiffCirc::computeLHS(const CartMesh *const m) const
{
	...
	if(rank == 0)	
		printf("ConvDiffCirc: ComputeLHS: Setting values of the LHS matrix...\n");
	...
	if(rank == 0)
		printf("ConvDiffCirc: ComputeLHS: Done.\n");
	
	return A;
}

inline void
ConvDiffCirc::assembly_kernel(const CartMesh *const m, const sint i, const sint j, const sint k,
                              const sint nghost,
