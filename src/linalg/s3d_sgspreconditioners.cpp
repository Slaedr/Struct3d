...
#pragma omp parallel default(shared) if(params.threadedapply)
	{
		for(int iswp = 0; iswp < params.napplysweeps; iswp++)
		{
			 ...
			 *
			 * In case of GCC 8.2, both auto-vectorization and OMP simd give the same number of iterations.
			 * However, OMP simd is a little bit faster. In any case, vectorization with GCC does not
			 * cause the number itertions to change.
			 *
			 * My guess is that any vectorization in GCC produces two versions of the vectorized loop,
			 * and it always ends up using the slower version with Not Much Vectorization (TM). Somehow,
			 * the "slower" version generated by OMP simd is a bit faster than the auto-vectorized one.
